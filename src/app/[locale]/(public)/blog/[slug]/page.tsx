...
export default async function BlogPostPage({ params }: Props) {
  ...
  const isInterview = post.category === "interview" && post.profileName;
  const profileName = isInterview ? localized(post, "profileName", locale) : "";
  const profileLangs: string[] = post.profileLanguages ? JSON.parse(post.profileLanguages) : [];
  const LANG_FLAGS: Record<string, { flag: string; label: Record<string, string> }> = {
    cs: { flag: "\u{1F1E8}\u{1F1FF}", label: { cs: "Čeština", uk: "Чеська", ru: "Чешский" } },
    uk: { flag: "\u{1F1FA}\u{1F1E6}", label: { cs: "Ukrajinština", uk: "Українська", ru: "Украинский" } },
    ru: { flag: "\u{1F1F7}\u{1F1FA}", label: { cs: "Ruština", uk: "Російська", ru: "Русский" } },
    en: { flag: "\u{1F1EC}\u{1F1E7}", label: { cs: "Angličtina", uk: "Англійська", ru: "Английский" } },
  };
  ...
  const jsonLd = [
    {
      ...
      ...(isInterview && {
        about: {
          "@type": "Person",
          name: profileName,
          ...(post.profileSalon && { worksFor: { "@type": "HairSalon", name: post.profileSalon } }),
          ...(post.profileCity && { address: { "@type": "PostalAddress", addressLocality: post.profileCity } }),
          ...(post.profileInstagram && { sameAs: [`https://instagram.com/${post.profileInstagram.replace("@", "")}`] }),
          ...(profileLangs.length && { knowsLanguage: profileLangs }),
        },
      }),
    },
    {
  ...
  return (
    <>
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Interview Profile Card — Premium */}
        {isInterview && (
          <>
            <div className="max-w-lg mx-auto mt-10 mb-2">
              <div className="relative rounded-2xl overflow-hidden bg-gradient-to-br from-[#fdf8f6] via-white to-[#f9f0ed] border border-rose/10 shadow-lg">
                {/* Decorative top accent */}
                <div className="h-24 bg-gradient-to-r from-rose/20 via-[#c9a9c5]/20 to-rose/10" />
                <div className="px-6 pb-6 -mt-14 text-center">
                  {/* Avatar */}
                  {post.profilePhoto ? (
                    <div className="w-28 h-28 rounded-full mx-auto mb-3 ring-[3px] ring-white shadow-lg overflow-hidden relative">
                      <Image src={post.profilePhoto} alt={profileName} fill sizes="112px" className="object-cover" />
                    </div>
                  ) : (
                    <div className="w-28 h-28 rounded-full mx-auto mb-3 ring-[3px] ring-white shadow-lg bg-gradient-to-br from-rose to-[#c9a9c5] flex items-center justify-center text-white text-3xl font-bold tracking-tight">
                      {profileName.split(" ").map((w) => w[0]).join("").slice(0, 2)}
                    </div>
                  )}

                  {/* Name & location */}
                  <h2 className="text-2xl font-bold text-ink tracking-tight">{profileName}</h2>
                  {(post.profileCity || post.profileSalon) && (
                    <p className="text-sm text-muted mt-1.5 flex items-center justify-center gap-1.5">
                      <svg className="w-3.5 h-3.5 text-rose/50" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15 10.5a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z" /></svg>
                      {[post.profileCity, post.profileSalon].filter(Boolean).join(" \u2022 ")}
                    </p>
                  )}
                  ...
                  {profileLangs.length > 0 && (
                    <div className="flex items-center justify-center gap-1.5 mt-3">
                      {profileLangs.map((code) => {
                        const info = LANG_FLAGS[code];
                        if (!info) return null;
                        return (
                          <span key={code} className="inline-flex items-center gap-1 px-2.5 py-1 bg-white/80 text-espresso rounded-full text-xs font-medium shadow-sm border border-line/30">
                            {info.flag} {info.label[locale] ?? info.label.cs}
                          </span>
                        );
                      })}
                    </div>
                  ...
                  <div className="flex flex-wrap items-center justify-center gap-2 mt-5">
                    {post.profileInstagram && (
                      <a href={`https://instagram.com/${post.profileInstagram.replace("@", "")}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-full bg-gradient-to-r from-purple-500 to-pink-500 text-white text-xs font-medium hover:shadow-md hover:scale-[1.02] transition-all">
                    ...
                    {post.profileTelegram && (
                      <a href={`https://t.me/${post.profileTelegram.replace("@", "")}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-full bg-sky-500 text-white text-xs font-medium hover:bg-sky-600 hover:scale-[1.02] transition-all">
                    ...
                    {post.profileWhatsapp && (
                      <a href={`https://wa.me/${post.profileWhatsapp.replace(/[^0-9]/g, "")}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-full bg-green-500 text-white text-xs font-medium hover:bg-green-600 hover:scale-[1.02] transition-all">
                    ...
                    {post.profilePhone && (
                      <a href={`tel:${post.profilePhone}`} className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-full bg-emerald-500 text-white text-xs font-medium hover:bg-emerald-600 hover:scale-[1.02] transition-all">
                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
                        {post.profilePhone}
                      </a>
                    )}
                    {post.profileWebsite && (
                      <a href={post.profileWebsite} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-full bg-espresso/80 text-white text-xs font-medium hover:bg-espresso hover:scale-[1.02] transition-all">
        ...
        {/* Content — interview gets special Q&A styling */}
        <article className={`py-8 max-w-none text-[15.5px] text-[#6b5e5a] leading-[1.9]
          ${isInterview
            ? `[&_h2]:text-[1.1rem] [&_h2]:font-bold [&_h2]:text-ink
               [&_h2]:mt-10 [&_h2]:mb-4 [&_h2]:px-4 [&_h2]:py-3
               [&_h2]:bg-gradient-to-r [&_h2]:from-[#fdf8f6] [&_h2]:to-transparent
               [&_h2]:rounded-xl [&_h2]:border-l-[3px] [&_h2]:border-l-rose
               [&_p]:mb-5 [&_p]:pl-1`
          ...
          <div className="mt-14">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {relatedPosts.map((r) => (
                <Link
                  key={r.id}
                  href={{ pathname: '/blog/[slug]' as any, params: { slug: r.slug } }}
