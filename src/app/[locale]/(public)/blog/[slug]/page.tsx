  ...
  return (
    <>
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {isInterview && (
          <>
            <div className="max-w-md mx-auto bg-white rounded-2xl border border-line shadow-sm p-6 text-center mt-8">
              {post.profilePhoto ? (
                <div className="w-28 h-28 rounded-full mx-auto mb-4 ring-2 ring-offset-2 ring-rose/30 overflow-hidden relative">
                  <Image src={post.profilePhoto} alt={profileName} fill sizes="112px" className="object-cover" />
                </div>
              ) : (
                <div className="w-28 h-28 rounded-full mx-auto mb-4 ring-2 ring-offset-2 ring-rose/30 bg-gradient-to-br from-rose/60 to-[#c9a9c5] flex items-center justify-center text-white text-2xl font-bold">
                  {profileName.split(" ").map((w) => w[0]).join("").slice(0, 2)}
                </div>
              )}
              <h2 className="text-xl font-bold text-ink">{profileName}</h2>
              ...
              <div className="flex items-center justify-center gap-3 my-4">
                {post.profileInstagram && (
                  <a href={`https://instagram.com/${post.profileInstagram.replace("@", "")}`} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-gradient-to-br from-purple-500 to-pink-500 text-white flex items-center justify-center hover:shadow-md transition-shadow" title="Instagram">
                ...
                {post.profilePhone && (
                  <a href={`tel:${post.profilePhone}`} className="w-10 h-10 rounded-full bg-emerald-500 text-white flex items-center justify-center hover:bg-emerald-600 transition-colors" title="Phone">
                    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
                  </a>
                )}
                {post.profileTelegram && (
                  <a href={`https://t.me/${post.profileTelegram.replace("@", "")}`} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-sky-500 text-white flex items-center justify-center hover:bg-sky-600 transition-colors" title="Telegram">
                ...
                {post.profileWhatsapp && (
                  <a href={`https://wa.me/${post.profileWhatsapp.replace(/[^0-9]/g, "")}`} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-green-500 text-white flex items-center justify-center hover:bg-green-600 transition-colors" title="WhatsApp">
                ...
                {post.profileWebsite && (
                  <a href={post.profileWebsite} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-gray-600 text-white flex items-center justify-center hover:bg-gray-700 transition-colors" title="Web">
              ...
              {profileLangs.length > 0 && (
                <div className="flex items-center justify-center gap-2 mt-3">
                  {profileLangs.map((code) => {
                    ...
                    if (!info) return null;
                    return (
                      <span key={code} className="inline-flex items-center gap-1 px-2 py-0.5 bg-nude-100 text-espresso rounded-full text-xs">
            ...
            <div className="border-t border-line/50 my-8" />
          </>
        )}
