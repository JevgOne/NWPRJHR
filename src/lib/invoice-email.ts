...
/**
 * Send deposit payment email — ONLY Comgate link, no bank details, no QR.
 * Client clicks → pays online → done.
 */
...
<body style="margin:0;padding:0;background-color:#fdfaf7;font-family:Georgia,'Times New Roman',serif;">
  ...
  const text = `${greeting},\n\npro potvrzení Vaší rezervace prosíme o úhradu zálohy ve výši ${amount}.\n\nZaplatit: ${opts.comgateUrl}\n\nDěkujeme, Hairland.cz`;
  ...
  await resend.emails.send({
    ...
    replyTo: "info@hairland.cz",
    to: opts.recipientEmail,
    subject: `Hairland — Záloha ${amount}`,
    text,
    html,
    headers: {
      "X-Entity-Ref-ID": `deposit-${opts.reservationNumber}`,
    },
  });
