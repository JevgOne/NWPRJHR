# TASK-11: Skladové pohyby padají — analýza a plán opravy
...
### Zjištěné příčiny (seřazeno dle pravděpodobnosti)

#### 1. HLAVNÍ PŘÍČINA: `pieces Int` bez defaultu ve StockMovement (schema:367)
...
### Krok 1: Opravit existující data v DB
Spustit SQL migration/fix pro staré záznamy:
```sql
-- Fix StockMovement records with NULL pieces
UPDATE stock_movements SET pieces = 0 WHERE pieces IS NULL;

-- Fix Delivery records with NULL initialPieces/remainingPieces
...
### Krok 2: Přidat default value do Prisma schema
```prisma
model StockMovement {
...
Pak `npx prisma db push` nebo migration.
...
### Krok 4: Opravit DB schema a push
```bash
npx prisma db push
```
